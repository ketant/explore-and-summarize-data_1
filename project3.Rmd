...
ggplot(data=ld, aes(x=LoanStatus, ymax = max(..count..)+5000)) + 
  ...
  scale_y_log10( breaks = trans_breaks("log10", function(x) 10 ^ x),
    ...
    stat_bin(geom="text", aes(label=..count.., vjust = -0.5 , position="dodge")) 
...
ggplot(data=ld,aes(x=BorrowerRate*100, ymax = max(..count..)+5000)) + 
...
ggplot(data=ld, aes(x=ProsperRating..Alpha., ymax = max(..count..)+5000)) + 
  geom_bar() +   
  scale_y_log10(breaks = trans_breaks( "log10", function(x) 10^x ), 
                labels = trans_format( "log10",  trans = math_format(10^.x))) +   
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + 
  stat_bin(geom="text", aes(label=..count.., vjust=-1)) 
...
ld.withoutCredit <- subset(ld, 
                           ld$ProsperRating..Alpha. =="" & ld$CreditGrade =="",
...
ggplot(data=ld, aes(x=DebtToIncomeRatio, ymax = max(..count..) + 30000 )) + 
...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.width=6, fig.height=7}
ggplot(data=ld, aes(x=LoanOriginationMonth, ymax =  max(..count..)+30000)) + 
  geom_bar() + 
  scale_y_log10(breaks = trans_breaks( "log10", function(x) 10^x ), 
                labels = trans_format( "log10",  trans = math_format(10^.x))) +   
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + 
...
ggplot(data=ld, aes(x=LoanOriginationDate)) + 
  geom_bar(binwidth=2) + 
  scale_x_date(breaks = date_breaks(width = "months"), labels = date_format("%b-%y")) + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) 
...
ggplot(data=ld, aes(x=ListingCategory..numeric.,y=..count..,ymax = max(..count..)+2000)) + 
  geom_bar() + 
...
ggpairs(ld.sample, 
        ...
        params = list(corSize=6,labelSize=4,binwidth=0.1)) +
...
ggplot(data=ld.state.creditgrade, 
...
ggplot(data=subset(ld,ProsperRating..Alpha.!=""), aes(x=ProsperRating..Alpha.,y=LoanOriginalAmount)) + 
  geom_boxplot() + 
...
ggplot(data=subset(ld, ld$CreditGrade != "") , 
       aes(x=BorrowerRate, fill=CreditGrade)) + 
  geom_bar() + 
  facet_wrap(~IncomeRange, ncol=2)

ggplot(data=subset(ld, ld$ProsperRating..Alpha. != "") , 
       aes(x=BorrowerRate, fill=ProsperRating..Alpha.)) + 
  geom_bar() + 
  facet_wrap(~IncomeRange, ncol=2)
...
ggplot(data=subset(ld,ProsperRating..Alpha.!=""), 
       aes(x = LoanOriginalAmount, y = EstimatedLoss*100, 
...
ggplot(data=subset(ld,ProsperRating..Alpha.!=""), aes(x = EstimatedLoss*100,
                    y = DebtToIncomeRatio, 
                    color = ProsperRating..Alpha.)) + 
  geom_point() + 
  ylim(0,1.0) +
...
ggplot(data=subset(ld,ProsperRating..Alpha.!=""), aes(x = LenderYield*100,
                    y = DebtToIncomeRatio, 
                    color = ProsperRating..Alpha.)) + 
  geom_point() + 
  ylim(0,1.5) +
...
range(
  subset(ld,
         ProsperRating..Alpha.!="" & DebtToIncomeRatio < 1.0 & ProsperRating..Alpha. == "AA", select = c(LenderYield))
  )
...
ggplot(data = subset(ld, 
                     LoanStatus %in% c("Chargedoff","Defaulted") & 
                       DebtToIncomeRatio < 1.0 ),
       aes( x = LoanOriginalAmount , y = DebtToIncomeRatio)) + 
         geom_point() + 
  facet_wrap(~ProsperRating..Alpha., ncol=2)

table(subset(ld, LoanStatus %in% c("Chargedoff","Defaulted") & DebtToIncomeRatio < 1.0 , select = c(ProsperRating..Alpha.)))
...
ggplot(data = subset(ld, 
       ...
       aes( x = LoanOriginalAmount , y = DebtToIncomeRatio)) + 
         geom_point() + 
  facet_wrap(~CreditGrade, ncol=3)

table(
  subset(ld, 
         LoanStatus %in% c("Chargedoff","Defaulted") & 
           DebtToIncomeRatio < 1.0, select = c(CreditGrade)
         )
  )
...
### Plot One
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.width=7, fig.height=20}
...
ggplot(data=ld, 
  ...
  scale_x_continuous(breaks=seq(0,20,1), 
                     labels=c("Not Available", "Debt Consolidation", 
                              ...
                              "Taxes","Vaction","Wedding Loans")) + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + 
  xlab("Listing Category") + 
  facet_wrap(~EmploymentStatus, ncol=1,scales="free_y")

table(
  subset(ld, 
         ld$EmploymentStatus=="Part-time",
...
ggplot(data=subset(ld, LoanStatus %in% c("Completed","FinalPaymentInProgress")), 
          aes(x=ProsperRating..Alpha., y=LenderYield*100)) + 
  ylim(5,35) +
  geom_boxplot() 
...
For Final plot 2, i subset the loans which were either completed or final payment phase to look at the historical lender yields by proper rating.

It appears that people with good credit grade will have a lower lender yield and vice versa.  
...
We can see that lender yield ranges from 3% to 16% for Borrowers with propser rating alpha of "AA" (Best Prosper Rating) and debt to income ratio below 1.0.  
I feel that there are good investment opportunities by funding prosper loans as the yields are attractive.  
...
ggplot(data=subset(ld, 
                   ProsperRating..Alpha.!=""&DebtToIncomeRatio<1.0 & LoanStatus %in% c("Chargedoff","Defaulted") ), 
          aes(x=ListingCategory..numeric.)) + 
  scale_x_continuous(breaks=seq(0,20,1), 
                     labels=c("Not Available", "Debt Consolidation", 
                              "Home Improvement","Business",
...
Final Plot 3 shows a subset of loans data which were defaulted or charged off.
...
also, we can see that these segment of borrowers with best prosper credit rating of "AA" have lender yields of 5% to 18.5%.  
...
I feel that, it will be interesting to build a model to determine prosper score to enrich the analysis.
